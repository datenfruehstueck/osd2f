<template>
  <div>
    <b-modal
      :title="content.consent_popup == undefined ? '' : content.consent_popup.title"
      id="consent-modal"
      :ok-title="content.consent_popup == undefined ? '' : content.consent_popup.accept_button"
      ok-variant="success"
      :cancel-title="content.consent_popup == undefined ? '' : content.consent_popup.decline_button"
      :ok-disabled="this.processing"
      @ok="sub">
        <b-overlay :show="show" rounded="sm">
          <div :aria-hidden="show ? 'true' : null">
            <div class="p-1"> {{ content.consent_popup == undefined ? '' : content.consent_popup.lead }} </div>
            <ul>
              <li v-for="lit in (content.consent_popup == undefined ? [] : content.consent_popup.points)"> {{lit}} </li>
            </ul>
            <div class="p-1"> {{ content.consent_popup == undefined ? '' : content.consent_popup.end_text }} </div>
          </div>
        </b-overlay>
    </b-modal>
  </div>
